...
const Checkout = () => {
  ...
  const generatePDF = () => {
    ...
    const addHeader = () => {
      ...
      doc.text("Shopping App Invoice", titleX + 15, titleY);
    ...
    // Table datacc
    const tableColumns = ["No.", "Item Name", "Num of Item", "Price"];
    ...
    doc.text(totalValue, doc.internal.pageSize.getWidth() - 30, y + 10, {
      align: "right",
    }); // Position total value on the right

    y += 20; // Move down for the next section

    // User information data
    const userColumns = ["Field", "Value"];
    const userRows = Object.keys(formData).map((item) => [
      item,
      `${formData[item]}`,
    ]);
